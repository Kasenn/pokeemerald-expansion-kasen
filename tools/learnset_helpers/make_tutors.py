...
def extract_repo_tutors() -> typing.Generator[str, None, None]:
    ...
    for inc_fname in chain(glob.glob("./data/scripts/*.inc"), glob.glob("./data/maps/*/scripts.inc")):
        with open(inc_fname, "r") as inc_fp:
            incfile = inc_fp.read()
            if not INCFILE_HAS_TUTOR_PAT.search(incfile) and not INCFILE_HAS_TUTOR_PAT2.search(incfile) and not INCFILE_HAS_TUTOR_PAT3.search(incfile):
                continue
            ...
            for move in INCFILE_MOVE_PAT3.finditer(incfile):
                yield move.group(1)
