...
#define AI_FLAG_WILD_CAN_FLEE               AI_FLAG(31)  // Makes it possible for a wild mon to flee an encounter
#define AI_FLAG_KNOW_OPPONENT_PARTY         AI_FLAG(32)  // AI knows all the species in the player's party, but not moves/items/abilities unless they've been seen.
#define AI_FLAG_RANDOMIZE_SWITCHIN          AI_FLAG(33)  // AI will randomly choose between eligible switchin candidates of a given category instead of picking the last one in the party.
#define AI_FLAG_RANDOMIZE_PARTY_INDICES     AI_FLAG(34)  // AI will randomize the order of the mons in its party, including the lead. Not an AI flag really, just a way to trigger TPP functionality
...
#define AI_FLAG_BASIC_TRAINER         (AI_FLAG_CHECK_BAD_MOVE | AI_FLAG_TRY_TO_FAINT)
