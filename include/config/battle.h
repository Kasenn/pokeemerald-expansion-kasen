...
#define B_UNEVOLVED_EXP_MULTIPLIER  GEN_5 // In Gen6+, if a Pokémon is at or past the level where it would be able to evolve, but it has not, it gains ~1.2x experience. This only applies to species with the EVO_LEVEL method.
#define B_LEVEL_UP_NOTIFICATION     GEN_5 // In Gen9+, if a Pokémon gains enough experience to level up multiple times, a message is only displayed once.
...
#define B_MOVE_TUTOR_COSTS          GEN_LATEST // Updates the default move tutor costs of some moves.
...
#define B_OBEDIENCE_MECHANICS           GEN_5 // In PLA onwards (GEN_8 here), obedience restrictions also apply to non-outsider Pokémon, albeit based on their met level rather than their current level.
...
#define B_SHOW_EFFECTIVENESS        SHOW_EFFECTIVENESS_NEVER // If not SHOW_EFFECTIVENESS_NEVER, the PP string is replaced by a type effectiveness indicator based on the move and the target.
...
#define B_SPRITE_BLEND_LEVEL                80       // How much the DNS blending affects the sprites on a 0-100 scale. 100 is fully applied DNS, 0 is no DNS blending at all
