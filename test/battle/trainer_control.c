...
TEST("CreateNPCTrainerPartyForTrainer generates customized Pokémon")
{
    ...
    u32 currTrainer = 0;
    u8 nickBuffer[20];
    CreateNPCTrainerPartyFromTrainer(testParty, &sTestTrainers[currTrainer], TRUE, BATTLE_TYPE_TRAINER);
...
TEST("Trainer Party Pool picks according to custom rules")
{
    struct Pokemon *testParty = Alloc(6 * sizeof(struct Pokemon));
    u32 currTrainer = 5;
    CreateNPCTrainerPartyFromTrainer(testParty, &sTestTrainers[currTrainer], TRUE, BATTLE_TYPE_TRAINER | BATTLE_TYPE_DOUBLE);
...
TEST("trainerproc supports both Double Battle: Yes and Battle Type: Doubles")
{
    ...
    const struct Trainer trainer = sTestTrainers[currTrainer];
