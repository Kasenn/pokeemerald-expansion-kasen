...
SINGLE_BATTLE_TEST("Mimicry changes the battler's type based on Terrain")
{
    ...
    GIVEN {
        ...
        switch (terrainMove)
        {
        case MOVE_ELECTRIC_TERRAIN: MESSAGE("The foe Stunfisk's type changed to Electric!"); break;
        case MOVE_PSYCHIC_TERRAIN:  MESSAGE("The foe Stunfisk's type changed to Psychic!"); break;
        case MOVE_GRASSY_TERRAIN:   MESSAGE("The foe Stunfisk's type changed to Grass!"); break;
        case MOVE_MISTY_TERRAIN:    MESSAGE("The foe Stunfisk's type changed to Fairy!"); break;
        }
    } THEN {
...
DOUBLE_BATTLE_TEST("Mimicry can trigger multiple times in a turn")
{
    GIVEN {
        ...
        MESSAGE("The foe Bagon used Electric Terrain!");
        ABILITY_POPUP(playerLeft, ABILITY_MIMICRY);
        MESSAGE("Stunfisk's type changed to Electric!");
        // igglybuff
        MESSAGE("The foe Igglybuff used Misty Terrain!");
        ABILITY_POPUP(playerLeft, ABILITY_MIMICRY);
