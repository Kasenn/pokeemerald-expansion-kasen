...
SINGLE_BATTLE_TEST("Protean/Libero changes the type of the user to the move used every time (Gen6-8)")
{
    ...
    GIVEN {
        ...
        if (species == SPECIES_KECLEON)
            ...
            MESSAGE("The foe Raboot transformed into the Water type!");
        ANIMATION(ANIM_TYPE_MOVE, MOVE_WATER_GUN, opponent);
        ABILITY_POPUP(opponent, ability);
        if (species == SPECIES_KECLEON)
            MESSAGE("The foe Kecleon transformed into the Normal type!");
        else
            MESSAGE("The foe Raboot transformed into the Normal type!");
...
SINGLE_BATTLE_TEST("Protean/Libero does not change the user's type when using Struggle")
{
    ...
    GIVEN {
        ...
        NONE_OF {
            ABILITY_POPUP(opponent, ability);
            MESSAGE("The foe Greninja transformed into the Normal type!");
            MESSAGE("The foe Raboot transformed into the Normal type!");
        }
