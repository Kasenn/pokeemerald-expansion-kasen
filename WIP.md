...
<!-- FIELD MOVES (ESPECIALLY ROCK CLIMB) -->
<!-- BAG -->
<!-- BATTLE PARTNERS AND THEIR PICS -->
<!-- TRAINERS -->
<!-- ABILITY POP-UP -->
...
<!-- FLASH AI -->
<!-- FIX INTRO RIVAL BATTLE WINNING TEXT -->
<!-- MEGA EXHAUSTION -->
<!-- MOVE_FLASH -->
<!-- BATTLE_TYPE_GHOST BALL USAGE -->
<!-- FIX ADAMANT TOWER RIVAL FACING DIRECTION -->
<!-- + RUNNING ANIMATION -->
<!-- AMPHY INTERRUPT, MAKE SURE THE SCRIPT IS FINE -->
<!-- SLEEP BERRY -->
<!-- RAPID FISTS -->
<!-- FORESTS CURSE -->
<!-- CACTUS COLOR -->
<!-- REGISTERED ITEMS X 3 -->
<!-- CHECK MONEY REWARD FOR SINGLE + DOUBLE -->
<!-- CHECK MONEY LOST FOR SINGLE + DOUBLE -->
<!-- ADD TIME-BASED BATTLE ENVIRONMENTS -->
<!-- ROUTE 18 LIGHTING -->
<!-- REWRITE HIVE LEADER! CHECK WRAP, INFESTATION, STATIC, POISON TOUCH FOR REFERENCE -->
<!-- FIX PRYCE AND BLAINE MUGSHOT -->
TM CASE		DestroyTMCaseBuffers
	fix border around selection
	fix callbacks + callback crash
ADD FREE SPACE BAG
MAYBE ADD CORRIN
...
everfrost town
	...
	celebi trainer
		...
		team
		info
	event with jasmine and volkner
route 25
	items
	weather
	trainers
...
frozen mines
	layout
	initial event with jasmine + volkner, split to 3 directions
	trainers
		dialogue
		scripts
		info
		teams
	items, 1-2 zygarde cells
	4-7 floors
	wild encounters
	rocket executives at the end
	rocket escapes with orb
galewind city
	pokecenter
	mart
	items
	...
	gym
		...
		leader jasmine
			...
			info
			team
			make swap to ampharos as ace
			postbattle, mention last gym
ocean walkway
	trainers
		dialogue			
		scripts
	...
	gate rooms
azuretide town
	old woman repair fragments into hm
		hm name pending
		psychic move
		fake out, increases speed if slower than opponent
		lets you time travel through portals
		portals do not appear until celebi has been awoken
		celebi event in the hidden glade

			how to catch celebi??
		portal locations:
			enchanted forest
				contains xerneas, won't open until post-game
			shoreslate
				pre-civilizations
			desert
				pre-yveltal destruction
			route 11
				fully forested
			sandstone city
				tower is there, city is not
			route 15
				covered in ice
			route 3
				dry ocean
			windplume valley
				high mountains, no wind
			route 27
				no snow
			route 16
				snowy, no lava, find a hole in the ground
					awaken heatran
				heatran appears in the original timeline (post-game)
			ocean skyway
				post-game, has cresselia
			pearlwood point
				no buildings
			route 23
				pre-collapse
