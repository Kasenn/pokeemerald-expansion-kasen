...
const struct RematchTrainer gRematchTable[REMATCH_TABLE_ENTRIES] =
{
    ...
    [REMATCH_JESSICA] = REMATCH(TRAINER_R6_TR5_HIKER, TRAINER_R8_TR3_AROMALADY, TRAINER_R8_TR4_YOUNGSTER, TRAINER_R8_TR5_PSYCHIC_F, TRAINER_R8_TR6_CAMPER_M, ROUTE121),
    ...
    [REMATCH_JACKI] = REMATCH(TRAINER_R12_TR13_NINJABOY, TRAINER_SANDSTONE_NURSE, TRAINER_R13_TR2_RUNNER_M, TRAINER_R13_TR3_EXPERTCOUPLE, TRAINER_R13_TR4_HIKER, ROUTE123),
    [REMATCH_WALTER] = REMATCH(TRAINER_R13_TR5_BLACKBELT, TRAINER_FC_TR3_CAMPER, TRAINER_FC_TR4_HIKER, TRAINER_R14_TR1_RUNNING_F, TRAINER_R14_TR2_HIKER, ROUTE121),
...
u8 GetTrainerBattleTransition(void)
{
    ...
    {
        if (gTrainerBattleOpponent_A == TRAINER_GABBY_AND_TY_1)
            return B_TRANSITION_SIDNEY2;
        if (gTrainerBattleOpponent_A == TRAINER_MARY)
            return B_TRANSITION_VOLKNER;
        if (gTrainerBattleOpponent_A == TRAINER_KAOLGYM_CLAY)
            return B_TRANSITION_CLAY;
        if (gTrainerBattleOpponent_A == TRAINER_LEADER_KORRINA)
            return B_TRANSITION_KORRINA;
        return B_TRANSITION_CHAMPION;
    }

    if (gTrainers[gTrainerBattleOpponent_A].trainerClass == TRAINER_CLASS_ELITE)
    {
        if (gTrainerBattleOpponent_A == TRAINER_WPV_TR14_SKYLA)
            return B_TRANSITION_SKYLA;
        return B_TRANSITION_CHAMPION;
    }
