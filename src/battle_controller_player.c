...
static void (*const sPlayerBufferCommands[CONTROLLER_CMDS_COUNT])(ubat battler) =
{
...
static void HandleInputChooseAction(enum BattlerId battler)
{
    ...
    // }
    else if (DEBUG_BATTLE_MENU == TRUE && JOY_NEW(L_BUTTON) && FlagGet(FLAG_KONAMI_CODE))
    {
...
static void PlayerHandleChooseAction(enum BattlerId battler)
{
    ...
    PREPARE_MON_NICK_BUFFER(gBattleTextBuff1, battler, gBattlerPartyIndexes[battler]);
    BattleStringExpandPlaceholdersToDisplayedString(gText_WhatWillPkmnDo);
    // BreakStringAutomatic(gDisplayedStringBattle, 96, 2, FONT_NORMAL);
