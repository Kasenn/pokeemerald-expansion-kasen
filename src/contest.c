...
bool8 SaveContestWinner(u8 rank)
{
    ...
    if (rank != CONTEST_SAVE_FOR_ARTIST)
    {
        ...
        gSaveBlock3Ptr->contestWinners[id].personality = gContestMons[i].personality;
        gSaveBlock3Ptr->contestWinners[id].species = gContestMons[i].species;
        gSaveBlock3Ptr->contestWinners[id].trainerId = gContestMons[i].otId;
        StringCopyN(gSaveBlock3Ptr->contestWinners[id].monName, gContestMons[i].nickname, VANILLA_POKEMON_NAME_LENGTH);
        StringCopy(gSaveBlock3Ptr->contestWinners[id].trainerName, gContestMons[i].trainerName);
        if(gLinkContestFlags & LINK_CONTEST_FLAG_IS_LINK)
            gSaveBlock3Ptr->contestWinners[id].contestRank = CONTEST_RANK_LINK;
        else
            gSaveBlock3Ptr->contestWinners[id].contestRank = gSpecialVar_ContestRank;

        if (rank != CONTEST_SAVE_FOR_MUSEUM)
            gSaveBlock3Ptr->contestWinners[id].contestCategory = gSpecialVar_ContestCategory;
        else
            gSaveBlock3Ptr->contestWinners[id].contestCategory = captionId;
    }
    else
    {
    ...
    case CONTEST_RANK_MASTER:
        if (shift)
        {
            for (i = NUM_CONTEST_HALL_WINNERS - 1; i > 0; i--)
                memcpy(&gSaveBlock3Ptr->contestWinners[i], &gSaveBlock3Ptr->contestWinners[i - 1], sizeof(struct ContestWinner));
        }
...
void ClearContestWinnerPicsInContestHall(void)
{
    ...
    for (i = 0; i < MUSEUM_CONTEST_WINNERS_START; i++)
        gSaveBlock3Ptr->contestWinners[i] = gDefaultContestWinners[i];
}
