    ...
    else
    {
        if (gBattleTypeFlags & BATTLE_TYPE_RECORDED_LINK)
        {
            if (gBattleTypeFlags & BATTLE_TYPE_MULTI)
                trainerPicId = GetBattlerLinkPlayerGender(battler) + TRAINER_BACK_PIC_BRENDAN;
            else
                trainerPicId = gLinkPlayers[gRecordedBattleMultiplayerId].gender + TRAINER_BACK_PIC_BRENDAN;
        }
        else
            trainerPicId = gLinkPlayers[0].gender + TRAINER_BACK_PIC_BRENDAN;
