...
static u16 FontFunc_Yoshi(struct TextPrinter *);
...
static void DecompressGlyph_Yoshi(u16, bool32);
...
static u32 GetGlyphWidth_Yoshi(u16, bool32);
...
static const u8 sYoshiArrowTiles[] = INCGFX_U8("graphics/fonts/down_arrow_yoshi.png", ".4bpp");
...
static const struct GlyphWidthFunc sGlyphWidthFuncs[] =
{
    ...
    { FONT_YOSHI,          GetGlyphWidth_Yoshi },
};
...
static u16 FontFunc_Yoshi(struct TextPrinter *textPrinter)
{
    if (textPrinter->hasFontIdBeenSet == FALSE)
    {
        textPrinter->fontId = FONT_YOSHI;
        textPrinter->hasFontIdBeenSet = TRUE;
    }
    return RenderText(textPrinter);
}
...
static u16 RenderText(struct TextPrinter *textPrinter)
{
    ...
    case RENDER_STATE_HANDLE_CHAR:
        ...
        case FONT_SHORT_NARROWER:
            ...
            break;
        case FONT_YOSHI:
            DecompressGlyph_Yoshi(currChar, textPrinter->japanese);
            break;
        case FONT_BRAILLE:
            break;
        }
...
static void DecompressGlyph_Yoshi(u16 glyphId, bool32 isJapanese)
{
    const u16 *glyphs;
    ...
    {
        glyphs = gFontYoshiGlyphs + (0x20 * glyphId);
        gCurGlyph.width = gFontYoshiGlyphWidths[glyphId];
...
static u32 GetGlyphWidth_Yoshi(u16 glyphId, bool32 isJapanese)
{
    if (isJapanese == TRUE)
        return gFontShortJapaneseGlyphWidths[glyphId];
    else
        return gFontYoshiGlyphWidths[glyphId];
}
