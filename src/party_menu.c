...
static void CreatePartyMonIconSpriteParameterized(enum Species species, u32 pid, bool32 isEgg, struct PartyMenuBox *menuBox, u8 priority, s8 slot);
...
static void CreatePartyMonIconSprite(struct Pokemon *, struct PartyMenuBox *, s8 slot);
...
static void CursorCb_TossEgg(u8);
...
static void CreatePartyMonSprites(u8 slot)
{
    ...
    if (gPartyMenu.menuType == PARTY_MENU_TYPE_MULTI_FULL_SHOWCASE && gPartyMenu.layout == PARTY_LAYOUT_MULTI_FULL_SHOWCASE_PARTNER)
    {
        ...
        {
            CreatePartyMonIconSpriteParameterized(gMultiPartnerParty[partySlot].species, gMultiPartnerParty[partySlot].personality, FALSE, &sPartyMenuBoxes[slot], 0, partySlot);
    ...
    {
        CreatePartyMonIconSprite(&party[partySlot], &sPartyMenuBoxes[slot], partySlot);
...
static void CreateCancelConfirmWindows(bool8 chooseHalf)
{
     ...
     && gPartyMenu.menuType != PARTY_MENU_TYPE_MULTI_FULL_SHOWCASE)
    {
        if (chooseHalf == TRUE)
        {
            ...
            AddTextPrinterParameterized4(confirmWindowId, FONT_SMALL, 2, 1, 0, 0, sFontColorTable[0], TEXT_SKIP_DRAW, gMenuText_Confirm);
...
static void DisplayPartyPokemonNickname(struct Pokemon *mon, struct PartyMenuBox *menuBox, u8 c, u32 wasChucked)
{
    ...
    if (GetMonData(mon, MON_DATA_SPECIES) != SPECIES_NONE)
    {
        ...
        if (GetMonData(mon, MON_DATA_SMART))
            StringCopy(nickname, sText_NewEgg);
...
static void DisplayPartyPokemonDescriptionText(u8 stringID, struct PartyMenuBox *menuBox, u8 c)
{
    ...
    if (c != 2)
        AddTextPrinterParameterized3(menuBox->windowId, FONT_NORMAL, menuBox->infoRects->descTextLeft, menuBox->infoRects->descTextTop - 2, sFontColorTable[0], 0, sDescriptionStringTable[stringID]);
}
...
static u8 DisplaySelectionWindow(u8 windowType)
{
    ...
    for (i = 0; i < sPartyMenuInternal->numActions; i++)
    {
        ...
        AddTextPrinterParameterized4(sPartyMenuInternal->windowId[0], GetFontIdToFit(text, FONT_NORMAL, 0, 64), cursorDimension, (i * 16) + 1, letterSpacing, 0, sFontColorTable[fontColorsId], 0, text);
    }
...
static void UNUSED CB2_ReturnToPartyMenuFromWritingMail(void)
{
...
static void UNUSED CB2_ReturnToPartyMenuFromReadingMail(void)
{
    gPaletteFade.bufferTransferDisabled = TRUE;
    InitPartyMenu(gPartyMenu.menuType, KEEP_PARTY_LAYOUT, gPartyMenu.action, TRUE, PARTY_MSG_DO_WHAT_WITH_MON, Task_TryCreateSelectionWindow, gPartyMenu.exitCallback);
}
...
static void CursorCb_TossEgg(u8 taskId)
{
    PlaySE(SE_SELECT);
    PartyMenuRemoveWindow(&sPartyMenuInternal->windowId[0]);
    Task_ClosePartyMenu(taskId);
...
static void CreatePartyMonIconSprite(struct Pokemon *mon, struct PartyMenuBox *menuBox, s8 slot)
{
    enum Species species = GetMonData(mon, MON_DATA_SPECIES);
    bool32 isEgg = GetMonData(mon, MON_DATA_IS_EGG);
    CreatePartyMonIconSpriteParameterized(species, GetMonData(mon, MON_DATA_PERSONALITY), isEgg, menuBox, 1, slot);
...
static void Task_TryItemUseFormChange(u8 taskId)
{
    ...
    case 1:
        if (gTasks[taskId].tAnimWait == 0)
        {
            ...
            CreatePartyMonIconSpriteParameterized(gTasks[taskId].tTargetSpecies, GetMonData(mon, MON_DATA_PERSONALITY), FALSE, &sPartyMenuBoxes[gPartyMenu.slotId], 1, gPartyMenu.slotId);
            icon->oam.mosaic = TRUE;
            icon->data[0] = 10;
            icon->data[1] = 1;
            icon->data[2] = taskId;
...
void TryItemHoldFormChange(struct Pokemon *mon, s8 slotId, enum BattleTrainer trainer)
{
    if (TryFormChange(mon, FORM_CHANGE_ITEM_HOLD, trainer))
    {
        ...
        CreatePartyMonIconSpriteParameterized(species, GetMonData(mon, MON_DATA_PERSONALITY), FALSE, &sPartyMenuBoxes[slotId], 1, gPartyMenu.slotId);
...
static void UNUSED CB2_ReturnToPartyOrBagMenuFromWritingMail(void)
{
    struct Pokemon *mon = &gParties[B_TRAINER_PLAYER][gPartyMenu.slotId];
    enum Item item = GetMonData(mon, MON_DATA_HELD_ITEM);
...
static bool8 GetBattleEntryEligibility(struct Pokemon *mon)
{
    ...
    if (GetMonData(mon, MON_DATA_SPECIES) == SPECIES_CHANSEY)
        return FALSE;
    if (GetMonData(mon, MON_DATA_IS_EGG))
        return TRUE;
    if (GetMonData(mon, MON_DATA_IS_EGG)
