...
static const u8 *GetInteractedMetatileScript(struct MapPosition *position, u8 metatileBehavior, enum Direction direction)
{
    ...
    if (MetatileBehavior_IsRockWall(metatileBehavior) == TRUE)
        return EventScript_UseRockClimb;
    if (MetatileBehavior_IsTrainerHillTimer(metatileBehavior) == TRUE)
        return EventScript_TrainerHillTimer;
    if (MetatileBehavior_IsSeedDrop(metatileBehavior) == TRUE)
        return EventScript_CutGrassSeedDrop;
...
static bool8 TryStartStepCountScript(u16 metatileBehavior)
{
    ...
    if (HiddenForestTakeStep() == TRUE)
    {
        ScriptContext_SetupScript(HiddenForestTriggerStarterSpawn);
        return TRUE;
    }
