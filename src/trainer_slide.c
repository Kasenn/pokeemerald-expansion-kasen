...
static const u8* const sTrainerSlides[TRAINER_PARTNER(PARTNER_COUNT)][TRAINER_SLIDE_COUNT] =
{
    ...
    [TRAINER_UNUSED_225_] = {[TRAINER_SLIDE_BEFORE_FIRST_TURN] = COMPOUND_STRING("Why hello there {PAUSE 60}")},
};
...
static const u8* const sTestTrainerSlides[MAX_TRAINERS_COUNT_EMERALD + PARTNER_COUNT][TRAINER_SLIDE_COUNT] =
{
...
{
#if TESTING
    return (FlagGet(TESTING_FLAG_TRAINER_SLIDES) ? sTestTrainerSlides[trainerId] : NULL);
#else
