...
{
    OpenPokenavForTownMap(CB2_ReturnToBagMenuPocket);
}
...
{
    gItemUseCB = ItemUseCB_IncreaseFriendship;
    SetUpItemUseCallback(taskId);
}
...
static u32 GetBallThrowableState(void)
{
    ...
    else if (FlagGet(WE_FLAG_NO_CATCHING) || !IsAllowedToUseBag() || gBattleTypeFlags & BATTLE_TYPE_MON_WITHOUT_TRAINER)
...
void ItemUseOutOfBattle_PokeFlute(u8 taskId)
{
    ...
    {
        if (!ExecuteTableBasedItemEffect(&gPlayerParty[i], ITEM_AWAKENING, i, 0, 0, 0))
            wokeSomeoneUp = TRUE;
    }
