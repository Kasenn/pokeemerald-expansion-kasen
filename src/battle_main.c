...
const struct TrainerClass gTrainerClasses[TRAINER_CLASS_COUNT] =
{
    ...
    [TRAINER_CLASS_DARKNESS_ROCKET]      = { _("Team Rocket"), 5},
    ...
    [TRAINER_CLASS_TUTOR_EV]         = { _("{PKMN} Center"), 0},
    [TRAINER_CLASS_TUTOR_EXP]         = { _("{PKMN} Center"), 0},
    ...
    [TRAINER_CLASS_MYSTERIOUS_SISTERS]           = { _("Mysterious Sisters"), 8},
    ...
    for (enum BattlerId battlerIndex = 0; battlerIndex < gBattlersCount; battlerIndex++)
    {
        ...
        case STATE_WAIT_ACTION_CHOSEN: // Try to perform an action.
            if (!IsBattleControllerActiveOrPendingSyncAnywhere(battler))
            {
                ...
                else if (gBattleTypeFlags & BATTLE_TYPE_NO_RUNNING
                         && gBattleResources->bufferB[battler][1] == B_ACTION_RUN)
                {
...
u32 GetBattlerTotalSpeedStat(enum BattlerId battler, enum Ability ability, enum HoldEffect holdEffect)
{
    ...
    if (ability == ABILITY_SWIFT_SWIM       && holdEffect != HOLD_EFFECT_UTILITY_UMBRELLA && weather  & B_WEATHER_RAIN)
        ...
        speed *= 2;
    else if (((gBattleMons[battler].types[0] == TYPE_FLYING) || (gBattleMons[battler].types[1] == TYPE_FLYING)) && weather & B_WEATHER_STRONG_WINDS)
        speed *= 2;
    ...
    if (holdEffect == HOLD_EFFECT_MACHO_BRACE || holdEffect == HOLD_EFFECT_POWER_ITEM)
        ...
        speed = (speed * 150) / 100;
    else if (holdEffect == HOLD_EFFECT_BOND_ANKLET && IsTradedMon(GetBattlerMon(battler)))
        speed = (speed * 120) / 100;
...
#else
    return (FlagGet(FLAG_WILDS_CAN_FLEE));
#endif
}
