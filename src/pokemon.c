    ...
    else // Player is the OT
    {
        ...
        else
        {
            ...
            if (MAP(MAP_FLOWER_FIELDS))
                totalRerolls += 8;
            if (VarGet(VAR_SHINY_MULTIPLIER) >= 100)
                totalRerolls = 0;
            else
...
bool8 ExecuteTableBasedItemEffect(struct Pokemon *mon, uit item, u8 partyIndex, u8 moveIndex, u8 modifyStats, u16 itemCount)
{
...
bool8 PokemonUseItemEffects(struct Pokemon *mon, uit item, u8 partyIndex, u8 moveIndex, bool8 usedByAI, u8 modifyStats, u16 itemCount)
{
    u32 dataUnsigned;
    ...
    if (species && species != SPECIES_EGG)
    {
        ...
        if (event == FRIENDSHIP_EVENT_LEAGUE_BATTLE)
        {
            ...
            if (!(opponentTrainerClass == TRAINER_CLASS_LEADER
                ...
                || opponentTrainerClass == TRAINER_CLASS_ELITE
...
enum TrainerPicID PlayerGenderToFrontTrainerPicId(ugen playerGender)
{   
    u8 trainerPic = 0;
    ...
    case OUTFIT_RUBYSAPPHIRE:
        if (playerGender != MALE)
            trainerPic = FACILITY_CLASS_RS_MAY;
        else
            trainerPic = FACILITY_CLASS_RS_BRENDAN;
        break;
    case OUTFIT_EMERALD:
        if (playerGender != MALE)
            trainerPic = FACILITY_CLASS_MAY;
        else
            trainerPic = FACILITY_CLASS_BRENDAN;
        break;
    case OUTFIT_ORAS:
        if (playerGender != MALE)
            trainerPic = FACILITY_CLASS_MAY_2;
        else
