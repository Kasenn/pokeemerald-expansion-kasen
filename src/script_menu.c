...
static u16 UNUSED GetLengthWithExpandedPlayerName(const u8 *str)
{
    ...
    while (*str != EOS)
    {
        if (*str == PLACEHOLDER_BEGIN)
        {
            ...
            {
                length += StringLength(gText_Yoshi);
                str++;
            }
        }
        else
...
{
    LoadMessageBoxAndFrameGfx(0, TRUE, 11);
...
static void CreateStartMenuForPokenavTutorial(void)
{
    ...
    AddTextPrinterParameterized(windowId, FONT_NORMAL, gText_Yoshi, 8, 73, TEXT_SKIP_DRAW, NULL);
