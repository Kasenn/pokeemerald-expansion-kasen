...
static s32 (*const sBattleAiFuncTable[])(enum BattlerId, enum BattlerId, enum Move, s32) =
{
    ...
    [34] = AI_WildCanFlee,           // Unused
    [35] = NULL,                     // AI_FLAG_ABILITY_OMNISCIENCE
    [36] = NULL,                     // AI_FLAG_ITEM_OMNISCIENCE
    [37] = NULL,                     // AI_FLAG_MOVE_OMNISCIENCE
    [37] = NULL,                     // Unused
    [38] = NULL,                     // Unused
    [39] = NULL,                     // Unused
    [40] = NULL,                     // Unused
...
// the following checks apply to any target (including user)
    ...
    case EFFECT_SUBSTITUTE:
        if (gBattleMons[battlerAtk].volatiles.substitute || aiData->abilities[battlerDef] == ABILITY_INFILTRATOR || aiData->abilities[battlerDef] == ABILITY_DATA_BREACH)
            ADJUST_SCORE(-8);
            ...
            case EFFECT_SWAGGER:
                if (gBattleMons[battlerAtkPartner].statStages[STAT_ATK] < MAX_STAT_STAGE
                 ...
                 && (!AI_CanBeConfused(battlerAtk, battlerAtkPartner, move, atkPartnerAbility)
                  ...
                  || atkPartnerHoldEffect == HOLD_EFFECT_LUM_BERRY))
                {
                    RETURN_SCORE_PLUS(WEAK_EFFECT);
                }
                break;
...
static s32 AI_CalcMoveEffectScore(enum BattlerId battlerAtk, enum BattlerId battlerDef, enum Move move, struct AiLogicData *aiData)
{
    ...
    case EFFECT_REST:
        ...
        else if (ShouldRecover(battlerAtk, battlerDef, move, 100))
        {
            if (aiData->holdEffects[battlerAtk] == HOLD_EFFECT_CURE_SLP
             || aiData->holdEffects[battlerAtk] == HOLD_EFFECT_LUM_BERRY
    ...
    case EFFECT_TOXIC_SPIKES:
        ...
        break;
    case EFFECT_FORESIGHT:
        if (aiData->abilities[battlerAtk] == ABILITY_SCRAPPY || aiData->abilities[battlerAtk] == ABILITY_MINDS_EYE || aiData->abilities[battlerAtk] == ABILITY_DATA_BREACH)
            break;
