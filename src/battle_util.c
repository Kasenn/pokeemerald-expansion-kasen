...
bool32 IsRockyTerrainAffected(enum BattlerId battler, enum Ability ability, enum HoldEffect holdEffect, enum BattleTerrain currTerrain)
{
    return IsBattlerTerrainAffected(battler, ability, holdEffect, B_TERRAIN_ROCKY, currTerrain);
}
...
static inline u32 CalcMoveBasePowerAfterModifiers(struct DamageContext *ctx)
{
    ...
    if (IsRockyTerrainAffected(battlerAtk, ctx->abilities[battlerAtk], ctx->holdEffects[battlerAtk], ctx->terrain) && moveType == TYPE_ROCK)
        modifier = uq4_12_multiply(modifier, (B_TERRAIN_TYPE_BOOST >= GEN_8 ? UQ_4_12(1.3) : UQ_4_12(1.5)));
    if (IsRockyTerrainAffected(battlerDef, ctx->abilities[battlerDef], ctx->holdEffects[battlerDef], ctx->terrain) && moveType == TYPE_WATER)
        modifier = uq4_12_multiply(modifier, UQ_4_12(0.5));
