        ...
        case ABILITY_HIVE_LEADER:
             ...
             && !CanBattlerAvoidContactEffects(gBattlerAttacker, gBattlerTarget, GetBattlerAbility(gBattlerAttacker), GetBattlerHoldEffect(gBattlerAttacker), move))
            {
                ...
                if (GetBattlerHoldEffect(gBattlerTarget) == HOLD_EFFECT_GRIP_CLAW)
                    ...
                    gBattleMons[gEffectBattler].volatiles.wrapTurns = B_BINDING_TURNS >= GEN_5 ? (Random() % 2) + 4 : (Random() % 4) + 2;
...
bool32 IsRockyTerrainAffected(enum BattlerId battler, enum Ability ability, enum HoldEffect holdEffect, u32 fieldStatuses)
{
    return IsBattlerTerrainAffected(battler, ability, holdEffect, fieldStatuses, STATUS_FIELD_ROCKY_TERRAIN);
}
...
bool32 HasEnoughHpToEatBerry(enum BattlerId battler, enum Ability ability, u32 hpFraction, enum Item itemId)
{
    ...
    if (hpFraction <= 4 && FreeSpace_GetItemPocket(itemId) == POCKET_BERRIES
         && gBattleMons[battler].hp <= gBattleMons[battler].maxHP / 2
...
static inline u32 CalcMoveBasePowerAfterModifiers(struct DamageContext *ctx)
{
    ...
    if (IsRockyTerrainAffected(battlerAtk, ctx->abilities[battlerAtk], ctx->holdEffects[battlerAtk], ctx->fieldStatuses) && moveType == TYPE_ROCK)
        modifier = uq4_12_multiply(modifier, (B_TERRAIN_TYPE_BOOST >= GEN_8 ? UQ_4_12(1.3) : UQ_4_12(1.5)));
    if (IsRockyTerrainAffected(battlerDef, ctx->abilities[battlerAtk], ctx->holdEffects[battlerDef], ctx->fieldStatuses) && moveType == TYPE_WATER)
        modifier = uq4_12_multiply(modifier, UQ_4_12(0.5));
    ...
    case HOLD_EFFECT_EVIOLITE:
        {
            ...
            && ((species != SPECIES_ARCANINE)
             ...
             || (species != SPECIES_BRAVIARY_OUTSIDER)
...
void UpdateMoveResultFlags(uq4_12_t modifier, u32 *resultFlags)
{
    if (modifier == UQ_4_12(0.0))
    {
        ...
        *resultFlags &= ~(MOVE_RESULT_SUPER_EFFECTIVE | MOVE_RESULT_NOT_VERY_EFFECTIVE);
...
u32 GetTotalAccuracy(struct BattleCalcValues *cv, u32 weather)
{
    ...
    if (cv->abilities[battlerAtk] == ABILITY_DATA_BREACH && GetBattleMoveType(cv->move) == TYPE_NORMAL)
        evasionStage = DEFAULT_STAT_STAGE;
