...
static void BuyMenuDrawGraphics(void)
{
    ...
    PrintMoneyAmountInMoneyBoxWithBorder(WIN_MONEY, 1, 13, 0);
    ScheduleBgCopyTilemapToVram(0);
...
static void BuyMenuCollectObjectEventData(void)
{
    ...
    for (y = 0; y < 5; y++)
    {
        for (x = 0; x < 7; x++)
        {
            ...
            if (objEventId != OBJECT_EVENTS_COUNT && !(gObjectEvents[objEventId].active && gObjectEvents[objEventId].graphicsId & OBJ_EVENT_MON && gObjectEvents[objEventId].localId != OBJ_EVENT_ID_FOLLOWER1))
            {
...
static void Task_BuyHowManyDialogueInit(u8 taskId)
{
    ...
    if (sShopData->totalCost == 0)
        ...
        maxQuantity = 1 / sShopData->totalCost;
