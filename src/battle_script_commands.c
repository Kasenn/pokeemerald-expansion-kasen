...
static bool32 TryDefogClear(enum BattlerId battlerAtk, bool32 clear)
{
    for (u32 i = 0; i < NUM_BATTLE_SIDES; i++)
    {
        ...
        {
            DEFOG_CLEAR(SIDE_STATUS_REFLECT, reflectTimer, BattleScript_SideStatusWoreOff2, MOVE_REFLECT);
            DEFOG_CLEAR(SIDE_STATUS_LIGHTSCREEN, lightscreenTimer, BattleScript_SideStatusWoreOff2, MOVE_LIGHT_SCREEN);
            DEFOG_CLEAR(SIDE_STATUS_MIST, mistTimer, BattleScript_MistWoreOff, MOVE_MIST);
...
static u32 ChangeStatBuffs(enum BattlerId battler, s8 statValue, enum Stat statId, union StatChangeFlags flags, u32 stats, const u8 *BS_ptr)
{
    ...
    if (statValue <= -1) // Stat decrease.
    {
        ...
        if (gSideTimers[GetBattlerSide(battler)].mistTimer
            ...
            && !(GetBattlerAbility(gBattlerAttacker) == ABILITY_DATA_BREACH && GetMoveType(gCurrentMove) == TYPE_NORMAL)
