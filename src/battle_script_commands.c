...
static void Cmd_getexp(void)
{
    ...
    case 2: // set exp value to the poke in expgetter_id and print message
        if (gBattleControllerExecFlags == 0)
        {
            ...
            else if (GetMonData(&gParties[B_TRAINER_PLAYER][*expMonId], MON_DATA_LEVEL) >= 90)
            {
                gBattleScripting.getexpState = 5;
                gBattleStruct->battlerExpReward = 0;
    ...
    case 3: // Set stats and give exp
        if (gBattleControllerExecFlags == 0)
        {
            ...
            if (GetMonData(&gParties[B_TRAINER_PLAYER][*expMonId], MON_DATA_HP) && currLvl < 90)
            {
...
static u32 UNUSED GetTrainerMoneyToGive(u16 trainerId)
{
    u32 lastMonLevel = 0;
    u32 moneyReward;
    u8 trainerMoney = 0;
...
void BS_JumpIfRoarFails(void)
{
    ...
    if (WILD_DOUBLE_BATTLE
        ...
        gBattlescriptCurrInstr = cmd->jumpInstr;
             ...
             && !IsOnPlayerSide(gBattlerTarget))
        gBattlescriptCurrInstr = cmd->jumpInstr;
    else if (FlagGet(WE_FLAG_NO_RUNNING) || gDisableRunning)
        gBattlescriptCurrInstr = cmd->jumpInstr;
    else
        gBattlescriptCurrInstr = cmd->nextInstr;
}
